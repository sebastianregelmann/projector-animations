...
func cancel_animation(cancelConfig: AnimationCancel) -> void:
    ...
    print("Cancel current animation and display default frame")
...
func count_animation_end():
    ...
    #stop animation playing
    animationRunning = false
    print("End animation and display default image")
    ...
    while file_name != "":
        if not dir.current_is_dir():
            ...
            if ext in ["png", "jpg", "jpeg"]:
                return load_texture_any_path(absolute_path.path_join(file_name))
...
func load_texture_any_path(path: String) -> Texture2D:
    if path.begins_with("res://") or path.begins_with("user://"):
        # Inside project or save folder → use regular load()
        return load(path)
    
    else:
        # Absolute file path → load manually
        var img := Image.new()
